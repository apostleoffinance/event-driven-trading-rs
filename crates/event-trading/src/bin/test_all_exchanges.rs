...
async fn main() -> Result<()> {
    ...
    test_decimal_conversions().context("Decimal conversion tests failed")?;
...
fn test_decimal_conversions() -> Result<()> {
    ...
    let decimal =
        Decimal::from_str(test_value).context(format!("Failed to parse {}", test_value))?;

    println!(
        "✅ Binance/Bybit format (from_str): {} → {}",
        test_value, decimal
    );
    ...
    let price1 = Decimal::from_str("100.50").context("Failed to parse price1")?;
    let price2 = Decimal::from_str("100.51").context("Failed to parse price2")?;
