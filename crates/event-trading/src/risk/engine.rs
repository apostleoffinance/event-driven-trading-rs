...
impl RiskEngine {
    ...
    pub fn pre_trade_validate(
        ...
        if entry_price <= Decimal::ZERO
            ...
            || stop_loss_distance <= Decimal::ZERO
        {
            return Err(TradingError::Validation(
        ...
        if !self
            .limits
            .can_open_new_position(self.portfolio.open_positions())?
        {
        ...
        if self.limits.is_leverage_exceeded(used_leverage)? {
            return Err(TradingError::Risk("Leverage exceeds limit".to_string()));
        }
    ...
    fn update_risk_state(&mut self) -> Result<()> {
        ...
        self.daily_loss = if equity < self.account_balance {
            loss
        } else {
            Decimal::ZERO
        };
