...
use super::order::{Order, OrderSide, OrderStatus, OrderType, TimeInForce};
use crate::engine::{Event, EventBus};
use crate::error::{Result, TradingError};
use crate::portfolio::position::PositionSide;
use crate::risk::{PortfolioLimits, PositionSizer, RiskEngine, StopLossManager};
...
impl ExecutionEngine {
    ...
    pub fn execute(
        ...
        let stop_loss =
            StopLossManager::calculate_stop_loss(entry_price, stop_loss_distance, is_long)?;
    ...
    pub fn cancel_order(&mut self, order_id: u64) -> Result<()> {
        let order = self
            .orders
            .get_mut(&order_id)
            .ok_or_else(|| TradingError::Execution("Order not found".to_string()))?;

        if matches!(order.status, OrderStatus::Filled | OrderStatus::Cancelled) {
            return Err(TradingError::Execution("Order already closed".to_string()));
        }
    ...
    fn process_fills(
        ...
        let fills = FillSimulator::simulate(order_id, &order.symbol, entry_price, order.quantity);
